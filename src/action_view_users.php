<?PHP
$tb = 'user'; 
$dir = 'uploads/passport/';
$dir_2 = 'uploads/valid_id/';

if ($_GET['v'] == true) {
	
	$arr_q = explode(' ',$_GET['q']);
	$mobile = array_pop($arr_q);
	$row_user = sql_select_one($db_con, $tb, 'mobile', $mobile);
	$_GET['q'] = $row_user['ID'];
	goto_page('view_user.php?v=true&q=' . $_GET['q']);
}

if ($_GET['e'] == true) {
	goto_page('edit_user.php?e=true&q=' . $_GET['q']);
}

if ($_GET['d'] == true)
{
	$db_res = sql_select_one($db_con, $tb, 'ID', $_GET['q']);
	if ($db_res['passport'])
		delete_file($db_res['passport'],$dir);
	if ($db_res['valid_id_file'])
		delete_file($db_res['valid_id_file'],$dir_2);
	
	Main::del_user($db_con, $_GET['q']);
	$db_res = sql_delete($db_con, $tb, 'ID', $_GET['q']);
	
	if ($db_res > 0) {		
		$error = 'Record deleted successfully';
		$errno = 200;
	} else {
		$error = 'Deleted record no longer exist';
		$errno = 300;
	}
}

$size = sql_count($db_con, $tb);
$pager = new Pager($size, 15);
$sql_stmt = 'SELECT * FROM `'.$tb.'` ORDER BY DATE DESC '.$pager->clause;

$db_res = sql_query($db_con, $sql_stmt);

if (is_array($db_res))
{		
	$sn = $pager->off; $row = ''; $rows = '';	
	foreach ($db_res as $key => $value)
	{
		$sn += 1;
		
		$row = '<tr>
			<td>
				<button class="btn btn-alt" onClick="onView('.$value['ID'].')" title="View">&#128065;</button>
			</td>
			<td>'. $sn .'</td>
			<td>'. ucwords($value['surname']) .'</td>
			<td>'. ucwords($value['names']) .'</td>
			<td>'. $value['gender'] .'</td>
			<td>'. null_f($value['email']) .'</td>
			<td>'. $value['mobile'].'<br/>'.$value['phone'] .'</td>
			<td>'. enum_f(Lists::STATE, $value['state']) .'</td>
			
			<td nw>'. date_f($value['reg_date']) .'</td>
			<td nw ar>
				<button class="btn btn-pri" onClick="onEdit('.$value['ID'].')" title="Edit">&#9998;</button>
				<button class="btn btn-sec" onClick="onDelete('.$value['ID'].')" title="Delete">&#10006;</button>
			</td>
		</tr>';

		$rows .= $row;
	}
}

$enum_bios = Main::bios($db_con);
$hint_bios = Enums::datalist($enum_bios, 'hint_bios');
?>