...
class Charts
{
	...
	public static function column ($caption, $labels, $values, $tilt = NULL) 
	{
		...
		for ($i = 0; $i < 2; $i++)
		{
			...
			for ($j = 0; $j < $cols; $j++)
			{
				$f = $i == 1? number_format($assoc[$i][$j]): $assoc[$i][$j];
				$row .= '<td '.$style.'>'.$f.'</td>';
			}
			$rows .= '<tr>'.$row.'</tr>'."\r\n";
		}
	...
	public static function column_pri ($caption, $labels, $values, $tilt = NULL) 
	{
		$style = is_null($tilt)? '': 'style="height:40px;font-size:10px;transform:rotate(-45deg);-webkit-transform:rotate(-45deg);-o-transform:rotate(-45deg);-moz-transform:rotate(-45deg);"';
		$cols = count($labels);
		$get_rates = self::get_rates($values);
		...
		for ($i = 100; $i >= 0; $i -= 10)
		{
			...
			for ($j = 0; $j <= $cols; $j++)
			{
				...
				{
					array_push($track, $rate);
					$row .= '<td class="active_pri" title="'.$label.', '.$value.'"></td>';
				}
				else
					$row .= '<td></td>';
			}
	...
	public static function column_sec ($caption, $labels, $values, $tilt = NULL) 
	{
		$style = is_null($tilt)? '': 'style="height:40px;font-size:10px;transform:rotate(-45deg);-webkit-transform:rotate(-45deg);-o-transform:rotate(-45deg);-moz-transform:rotate(-45deg);"';
		$cols = count($labels);
		$get_rates = self::get_rates($values);
		...
		for ($i = 100; $i >= 0; $i -= 10)
		{
			...
			for ($j = 0; $j <= $cols; $j++)
			{
				...
				{
					array_push($track, $rate);
					$row .= '<td class="active_sec" title="'.$label.', '.$value.'"></td>';
				}
				else
					$row .= '<td></td>';
			}
	...
	public static function ring_demo ($width = 9)
	{
		switch ($width) {
			case 1:
				$rate = 85; break;
		...
		return self::ring('Ring Chart','Battery Life',$rate, $width);
	}
	public static function ring ($caption, $label, $rate, $width = 2) 
	{
		$color = self::get_color($rate);
		if ($rate < 0) {
	...
	public static function ring_dark ($caption, $label, $rate, $width = 2) 
	{
		$color = self::get_color($rate);
		if ($rate < 0) {
			$append = 'chart_ring_negative';
		...
		$chart = '<table border="0" class="chart chart_ring">
			...
			<tr>
				<td>
					<svg viewbox="0 0 36 36.9" width="250" height="250" xmlns="http://www.w3.org/2000/svg">
						<circle stroke="#2F3E55" stroke-width="'.$width.'" fill="none" cx="17.9" cy="18.5" r="15.9" />
						<circle class="chart_ring '.$append.'" stroke-dasharray="'.$rate.',100" stroke="'.$color.'" 
							stroke-width="'.$width.'" fill="none" stroke-linecap="round" cx="17.9" cy="18.5" r="15.9" />
						<g class="chart_ring_inner">
							<text class="chart_ring_rate" fill="#FFF" x="18" y="20" text-anchor="middle">'.$rate.'%</text>
							<text class="chart_ring_label" fill="#CCC" x="18" y="24"	text-anchor="middle">'.$label.'</text>
						</g>
					</svg>
				</td>
			</tr>
